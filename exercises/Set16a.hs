...
sumIsLength input output = length input === foldr (\x n -> snd x + n) 0 output
...
inputInOutput input output = forAll (elements input) (\x -> elem x [fst y | y <- output])
...
outputInInput input output =
  forAll (elements output) (\x -> snd x == foldr (\z i -> if z == fst x then i+1 else i) 0 input)
...
instance Arbitrary Arg where
  arbitrary = do
    ...
    arg <- elements [Number n, Variable c]
    pure arg
...
instance Arbitrary Expression where
  arbitrary = do
    arg1 <- arbitrary
    arg2 <- arbitrary
    expr <- elements [Plus arg1 arg2, Minus arg1 arg2]
    pure expr
