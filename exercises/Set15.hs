...
statements xs ys = say <$> xs <*> ys <*> ["", "not "]
  where
...
calculator op num = checkOp op *> if op == "double" then (*2) <$> readMaybe num else (0-) <$> readMaybe num
  where
    checkOp :: String -> Maybe String
    checkOp op = if op == "double" || op == "negate" then Just op else Nothing
...
validateDiv n d = liftA2 div checkedN checkedD
  where
    checkedN = check True "" n
...
validateAddress streetName streetNumber postCode =
  ...
  check (go streetNumber) "Invalid street number" streetNumber <*>
  check (length postCode == 5 && go postCode) "Invalid postcode" postCode
  where
    go :: String -> Bool
    go sn = case (readMaybe sn :: Maybe Int) of
      Just _ -> True
      Nothing -> False
...
boolOrInt s = checkIfBool s <|> checkIfInt s

checkIfBool :: String -> Validation (Either Bool Int)
checkIfBool s = check (s == "True" || s == "False") "Not a Bool" (if s == "True" then Left True else Left False)

checkIfInt :: String -> Validation (Either Bool Int)
checkIfInt s = check (go s) "Not an Int" (Right $ read s)
  where
    go :: String -> Bool
    go sn = case (readMaybe sn :: Maybe Int) of
      Just _ -> True
...
normalizePhone pn =
  let
    strippedPn = [n | n <- pn, n /= ' ']
  in
    validatePhone strippedPn
  where
    validatePhone :: String -> Validation String
    validatePhone stripped =
      pure stripped <*
      checkLength stripped <*
      checkInvalidChars stripped
    checkLength :: String -> Validation String
    checkLength stripped = check (length stripped <= 10) "Too long" stripped
    checkInvalidChars :: String -> Validation String
    checkInvalidChars [] = pure []
    checkInvalidChars (x:xs) = liftA2 (:) checkThis checkRest
      where
        checkThis = check (isDigit x) ("Invalid character: " ++ [x]) x
        checkRest = checkInvalidChars xs
...
parseExpression e =
  if length (words e) /= 3 then
  check False ("Invalid expression: " ++ e) (Plus (Number 0) (Number 0))
  else
  parseOp (words e !! 1) <*>
  parseArg (head (words e)) <*>
  parseArg (last (words e))
  
parseOp :: String -> Validation (Arg -> Arg -> Expression)
parseOp op =
  check (op == "+" || op == "-") ("Unknown operator: " ++ op) (if op == "+" then constrPlus else constrMinus)

constrPlus :: Arg -> Arg -> Expression
constrPlus x y = Plus x y

constrMinus :: Arg -> Arg -> Expression
constrMinus x y = Minus x y

parseArg :: String -> Validation Arg
parseArg a =
  check (all isDigit a) ("Invalid number: " ++ a) (Number (read a)) <|>
  check (length a == 1 && isAlpha (head a)) ("Invalid variable: " ++ a) (Variable $ head a)
