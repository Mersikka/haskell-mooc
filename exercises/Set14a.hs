...
greetText str
  | T.length str > 15 = T.pack "Hello, " <> T.take 15 str <> T.pack "...!"
  | otherwise = T.pack "Hello, " <> str <> T.pack "!"
...
shout :: T.Text -> T.Text
shout str = T.unwords $ go (T.words str) True
  where
    go :: [T.Text] -> Bool -> [T.Text]
    go [] _ = []
    go (w:ws) b = if b then T.toUpper w : go ws False else w : go ws True
...
longestRepeat str = go str Nothing 0 0
  where
    go :: T.Text -> Maybe Char -> Int -> Int -> Int
    go str Nothing curr best =
      case T.uncons str of
        Nothing -> max curr best
        Just (c,rest) -> go rest (Just c) (curr+1) (max (curr+1) best)
    go str (Just last) curr best =
      case T.uncons str of
        Nothing -> max curr best
        Just (c,rest) -> if c == last then go rest (Just c) (curr+1) (max (curr+1) best)
                                      else go rest (Just c) 1 (max (curr) best)
...
takeStrict i str = TL.toStrict $ TL.take i str
...
byteRange bs = if (length $ B.unpack bs) > 1 then (maximum $ B.unpack bs) - (minimum $ B.unpack bs) else 0
...
xorChecksum bs = foldr (\b s -> Data.Bits.xor b s) 0 $ B.unpack bs
...
countUtf8Chars str =
  case Data.Text.Encoding.decodeUtf8' str of
    (Left _) -> Nothing
...
pingpong bs = BL.pack $ cycle $ B.unpack bs ++ B.unpack (B.reverse bs)
