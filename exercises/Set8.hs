...
combine f (Picture g) (Picture h) = Picture go
  where
    go c = f (g c) (h c)
...
paintSolid color shape (Picture g) = Picture f
  where
    f (Coord x y) = if contains shape x y then color else g $ Coord x y
...
paint (Picture g) shape (Picture h) = Picture f
  where
    f (Coord x y) = if contains shape x y then g (Coord x y) else h (Coord x y)
------------------------------------------------------------------------------
...
instance Transform Blur where
  apply Blur (Picture h) = Picture f
    where
      f (Coord x y) = go (h (Coord x y)) (h (Coord (x+1) y)) (h (Coord (x-1) y)) (h (Coord x (y+1))) (h (Coord x (y-1)))
        where
          go (Color r1 g1 b1) (Color r2 g2 b2) (Color r3 g3 b3) (Color r4 g4 b4) (Color r5 g5 b5) =
