...
treeMax Empty = 0
treeMax (Node v c1 c2) = max v (max (treeMax c1) (treeMax c2))
...
allValues f (Node v c1 c2) = if f v then allValues f c1 && allValues f c2 else False
...
mapTree f (Node v c1 c2) = Node (f v) (mapTree f c1) (mapTree f c2)
...
cull _ Empty = Empty
cull val (Node v c1 c2) = if val == v then Empty else Node v (cull val c1) (cull val c2)
...
isOrdered (Node v (Node v1 c11 c12) Empty) = v1 <= v && isOrdered (Node v1 c11 c12)
isOrdered (Node v Empty (Node v2 c21 c22)) = v <= v2 && isOrdered (Node v2 c21 c22)
isOrdered (Node v (Node v1 c11 c12) (Node v2 c21 c22)) = v1 <= v && v <= v2 && isOrdered (Node v1 c11 c12) && isOrdered (Node v2 c21 c22)
...
walk (StepL:ss) (Node _ c1 _) = walk ss c1
walk (StepR:ss) (Node _ _ c2) = walk ss c2
...
set [] val Empty = Empty
set [] val (Node _ c1 c2) = Node val c1 c2
set (StepL:ss) val (Node v c1 c2) = Node v (set ss val c1) c2
set (StepR:ss) val (Node v c1 c2) = Node v c1 (set ss val c2)
