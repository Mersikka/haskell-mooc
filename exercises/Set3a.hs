...
firstHalf s = if length s `mod` 2 == 0 then take (length s `div` 2) s else take ((length s `div` 2) + 1) s
...
whileRight check x = case check x of Right y -> whileRight check y
                                     Left y -> y
...
joinToLength l xs = [first ++ last | first <- xs, last <- xs, (==) l . length $ first ++ last]
...
interpreter (c:cs) = interpreter' c cs 0 0

interpreter' :: String -> [String] -> Int -> Int -> [String]
interpreter' cmd [] x y = case cmd of
  "printX" -> show x : []
  "printY" -> show y : []
  otherwise -> []
interpreter' cmd (c:cmds) x y = case cmd of
  "printX" -> show x : interpreter' c cmds x y
  "printY" -> show y : interpreter' c cmds x y
  "up"     -> interpreter' c cmds x (y+1)
  "down"   -> interpreter' c cmds x (y-1)
  "right"  -> interpreter' c cmds (x+1) y
  "left"   -> interpreter' c cmds (x-1) y
