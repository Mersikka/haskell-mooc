...
split s = case find (\x -> snd x == ' ') (zip [0..] s) of (Just y) -> Just (take (fst y) s, drop (fst y+1) s)
                                                          Nothing -> Nothing
...
checkNumber (f, s) = if go f || go s then Nothing else Just (f,s)
  where
    go n = any (\x -> elem x ['1','2','3','4','5','6','7','8','9','0']) n
...
checkCapitals (f, s) = if isLower (head f) || isLower (head s) then Nothing else Just (f,s)
...
winner scores player1 player2 = do s1 <- lookup player1 scores
                                   s2 <- lookup player2 scores
                                   case compare s1 s2 of EQ -> pure player1
                                                         GT -> pure player1
                                                         LT -> pure player2
...
selectSum xs is = fmap sum . sequence $ map (safeIndex xs) is

safeIndex :: [a] -> Int -> Maybe a
safeIndex xs i = lookup i (zip [0..] xs) >>= pure
...
countAndLog p xs = Logger (map show ys) (length ys)
...
rob from to = BankOp robHelper
  where
    robHelper :: Bank -> ((),Bank)
    robHelper (Bank accs) =
      let
        fromBalance = Map.findWithDefault 0 from accs
        newAccs = Map.adjust (\x -> x+fromBalance) to (Map.adjust (const 0) from accs)
      in ((), Bank newAccs)
...
count x = do st <- get
             case go x st [] of (Just newst) -> put newst
                                Nothing      -> put $ st ++ [(x,1)]
  where
    go :: Eq a => a -> [(a,Int)] -> [(a,Int)] -> Maybe [(a,Int)]
    go x [] _ = Nothing
    go x (y:ys) res
      | x == fst y = Just (res ++ [(x,snd y + 1)] ++ ys)
      | otherwise  = go x ys (y:res)
