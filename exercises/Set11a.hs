...
readWords 0 = return []
readWords n = do answer <- getLine
                 answers <- readWords (n-1)
                 return (sort (answer:answers))
...
readUntil f = go f []
    where
        go f rs = do a <- getLine
                     if f a then return rs else go f (rs++[a])
